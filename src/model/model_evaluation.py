...
def main():
    ...
    with mlflow.start_run() as run:
        try:
            ...
            # Log model with signature and trusted types
            mlflow.sklearn.log_model(
                sk_model=model,
                artifact_path="lgbm_model",
                signature=signature,  
                input_example=input_example,
                skops_trusted_types=trusted_types # <--- Added for LightGBM compatibility
            )
            ...
            # 1. Log overall accuracy directly (since it's a float, not a dict)
            if 'accuracy' in report:
                mlflow.log_metric("test_accuracy", report['accuracy'])

            # 2. Log classification report metrics for individual classes and averages
            for label, metrics in report.items():
                if isinstance(metrics, dict):
                    mlflow.log_metrics({
                        f"test_{label}_precision": metrics['precision'],
